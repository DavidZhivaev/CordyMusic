...
class Embeds(Embed):
    ...
    async def simple(
        self, 
        interaction: Union[Context, ApplicationCommandInteraction] = None, 
        image: str = None, 
        thumbnail: str = None, 
        footer: dict = None,
        fields: list = None,
        ...
        if fields:
            for field in fields:
                with suppress(Exception):
                    embed.add_field(name=field.get('name'), value=field.get('value'), inline=field.get('inline') if field.get('inline') else None) # pyright: ignore[reportArgumentType]
